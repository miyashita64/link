<section class="profile-area d-flex flex-column">
    ...
    <section class="profile-head">
        <a href="./home?child_id={{ $datas["child"]->id }}">&lt;</a>
        <span>プロフィールの設定</span>
    </section>
    <section class="profile-body">
        <form method="POST" action="./update_child" enctype="multipart/form-data">
            @csrf
            <p>子供のプロフィール<input type="submit" value="更新" onclick="alertAndSubmit(event)"></p>
            <input type="hidden" name="id" value="{{ $datas["child"]->id }}">
            <label>名前</label>
            <input type="text" name="name" value="{{ $datas["child"]->name }}">
            <label>アイコン</label>
            <div class="icon-change-cover">変更</div>
            <input type="file" name="icon_img" style="width: 80px; height: 80px; margin-top: -80px; opacity: 0;" accept="image/*">
            <img src="{{ $datas["child"]->icon_path }}">
        </form>

        <form method="POST" action="./update_parent" enctype="multipart/form-data">
            @csrf
            <p>保護者のプロフィール<input type="submit" value="更新" onclick="alertAndSubmit(event)"></p>
            <label>名前</label>
