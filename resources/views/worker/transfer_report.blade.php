...
@section('content')
<div id="content">
    <form id="service-data" method="POST" action='./output_transfer_excel_report'>
        @csrf
    ...
    @php
        ...
        foreach($datas["clients"] as $client){
            $list_datas[] = [
                "url" => "./transfer_report_view?facilitie_id=". $datas["facilitie"]["id"] ."&client_id=". $client["id"] ."&date=". $datas["date"],
                "onclick" => "getExcelReport(".$client['id'].")",
                "img_path" => $client["icon_path"],
                "title" => $client["name"],
                "subtitle" => "Excelファイルをダウンロード"
            ];
        }
    @endphp
